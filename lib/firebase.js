...
function fromFirestore(doc) {
  ...
  for (const [key, val] of Object.entries(doc.fields)) {
    if ('stringValue' in val) result[key] = val.stringValue;
      ...
      'mapValue' in v ? fromFirestore(v.mapValue) : (v.stringValue ?? v.integerValue ?? v.booleanValue ?? null)
    );
...
// Wrap db so .collection(name) auto-prefixes with TENANT_ID when set
...
export const db = withTenant(_rawDb);
