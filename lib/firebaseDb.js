...
function fromFirestore(doc) {
  ...
  for (const [k, v] of Object.entries(doc.fields)) {
    ...
    else if ('arrayValue' in v) r[k] = (v.arrayValue.values || []).map(x =>
      'mapValue' in x ? fromFirestore(x.mapValue) :
      x.stringValue ?? x.integerValue ?? x.booleanValue ?? null
    );
...
// Wrap a db-like object so .collection(name) auto-prefixes with TENANT_ID when set
function withTenant(base) {
  return {
    collection(name) {
      const prefix = process.env.TENANT_ID;
      return base.collection(prefix ? `${prefix}_${name}` : name);
    }
  };
}

export const fdb = withTenant(_rawFdb);

// Re-export base db wrapped with tenant prefix for backward compat
export const db = withTenant(baseDb);
