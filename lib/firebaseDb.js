  ...
  collection(col) {
    return {
      ...
      async where(field, op, value) {
        ...
        const resp = await fetch(`${BASE_URL.replace('/documents', '')}/:runQuery`, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
