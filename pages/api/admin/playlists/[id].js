...
export default async function handler(req, res) {
  ...
  if (req.method === 'PATCH') {
    ...
    if (action === 'reorder' && Array.isArray(songs)) {
      ...
      await db.collection('playlists').doc(id).update({ songs, updatedAt: new Date().toISOString() });
    ...
    if (action === 'add-song') {
      ...
      await db.collection('playlists').doc(id).update({ songs: updated, updatedAt: new Date().toISOString() });
      return res.json({ success: true });
    }
    ...
    if (action === 'remove-song') {
      ...
      await db.collection('playlists').doc(id).update({ songs: filtered, updatedAt: new Date().toISOString() });
      return res.json({ success: true });
    }

    if (name) {
      await db.collection('playlists').doc(id).update({ name, updatedAt: new Date().toISOString() });
      return res.json({ success: true });
    }
